            ...
            <div class="col-md-7">
                ...
                <div class="feature_article_wrapper">
                    ...
                    <div class="feature_article_inner">
                        ...
                        <a href="{{url('/author')}}/{{$hot_news->creator->id}}">{{$hot_news->creator->name}}</a>,<a href="#" target="_self">
...
@foreach ($top_viewed as $item)
            <div class="col-md-5" style="margin-bottom: 3%">
                <div class="feature_static_wrapper">
                    <div class="feature_article_img">
                        <img class="img-responsive" src="{{ asset('post') }}/{{ $item->main_image }}" alt="{{ $item->title }}">
                    </div>
                    <!-- feature_article_img -->
                    <div class="feature_article_inner">
                    ...
                    <div class="feature_article_date"><a href="{{url('/author')}}/{{$item->creator->id}}">{{$item->creator->name}}</a>,
                    {{date('F j,Y',strtotime($item->created_at))}}</div>
                        <!-- feature_article_date -->

                    <div class="feature_article_content">{{$item->short_description}}</div>
                        <!-- feature_article_content -->
                        <div class="article_social">
...
<div class="category_section mobile">
    ...
    <h2><a href="{{url('/details')}}/{{ $category->id }}">{{ $category->name }}</a></h2>
    </div>
    <!----article_title------>
    ...
    <div class="category_article_wrapper">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div class="category_article_date"><a href="{{url('/author')}}/{{$item->creator->id}}">{{$item->creator->name}}</a>,
                    {{date('F j,Y',strtotime($item->created_at))}}</div>
            ...
            <div class="col-md-6" style="margin-top: 3%">
                <div class="media">
                    <div class="media-left">
                        <a href="{{url('/details')}}/{{$item->slug}}"><img class="media-object" src="{{ asset('post') }}/{{ $item->thumb_image }}" alt="{{$item->title}}"
                        alt="{{$item->title}}"></a>
                    </div>
                    ...
                    <h3 class="media-heading"><a href="{{url('/details')}}/{{$item->slug}}">{{$item->title}}</a></h3>
                        <span class="media-date"><a href="{{url('/author')}}/{{$item->creator->id}}">{{$item->creator->name}}</a>,
                            {{date('F j,Y',strtotime($item->created_at))}}</span>
